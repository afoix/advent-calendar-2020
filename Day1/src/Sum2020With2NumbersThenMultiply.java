...
public class Sum2020With2NumbersThenMultiply {
    ...
    public static void main(String args[]) throws FileNotFoundException {
        ...
        for (int numberA = 0; numberA < numbers.size(); numberA++ ){
            ...
            for (int numberB = numberA + 1; numberB < numbers.size(); numberB++){
                compareNumbers(numbers.get(numberA), numbers.get(numberB));
            }
        }
    }

    private static void compareNumbers(Integer numberA, Integer numberB) {
        Integer sumOfNumbers =  numberA + numberB;

        if (sumOfNumbers == 2020) {
            Integer product = numberA * numberB;
            System.out.println(String.format("The numbers are %d and %d. The product is %d", numberA, numberB, product));
            return;
        }
    }
